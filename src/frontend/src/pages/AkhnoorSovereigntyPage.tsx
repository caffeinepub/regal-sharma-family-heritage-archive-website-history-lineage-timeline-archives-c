...
import { akhnoorSovereigntyContent } from '../content/historyContent';
import SourcesCitationsBlock from '../components/common/SourcesCitationsBlock';

export default function AkhnoorSovereigntyPage() {
  useEffect(() => {
    document.title = 'Akhnoor Sovereignty - The Sharma Family Heritage Archive';
    const metaDescription = document.querySelector('meta[name="description"]');
    if (metaDescription) {
      metaDescription.setAttribute('content', 'The grant of autonomy over Akhnoor district and the establishment of the Rajas of Akhnoor.');
    }
  }, []);
      ...
      <div className="container mx-auto px-4 max-w-4xl">
        <div className="mb-8">
          ...
          <h1 className="font-serif text-4xl md:text-5xl font-bold text-heritage-burgundy mb-4">
            {akhnoorSovereigntyContent.title}
          </h1>
          <p className="text-xl text-muted-foreground italic">
            {akhnoorSovereigntyContent.subtitle}
          </p>
        </div>
        ...
        <div className="prose prose-lg max-w-none space-y-8">
          {akhnoorSovereigntyContent.sections.map((section, index) => (
        ...
        {akhnoorSovereigntyContent.futureResearchPlaceholder && (
