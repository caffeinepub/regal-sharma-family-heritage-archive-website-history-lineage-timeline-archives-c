...
import { royalAdministrationContent } from '../content/historyContent';
import SourcesCitationsBlock from '../components/common/SourcesCitationsBlock';

export default function RoyalAdministrationLegacyPage() {
  useEffect(() => {
    document.title = 'Royal Administration & Legacy - The Sharma Family Heritage Archive';
    const metaDescription = document.querySelector('meta[name="description"]');
    if (metaDescription) {
      metaDescription.setAttribute('content', 'The administrative and cultural contributions of the Sharma family under the Dogra rulers of Jammu & Kashmir.');
    }
  }, []);
      ...
      <div className="container mx-auto px-4 max-w-4xl">
        <div className="mb-8">
          <h1 className="font-serif text-4xl md:text-5xl font-bold text-heritage-burgundy mb-4">
            {royalAdministrationContent.title}
          </h1>
          <p className="text-xl text-muted-foreground italic">
            {royalAdministrationContent.subtitle}
          </p>
        </div>
        ...
        <div className="prose prose-lg max-w-none space-y-8">
          {royalAdministrationContent.sections.map((section, index) => (
        ...
        {royalAdministrationContent.futureResearchPlaceholder && (
